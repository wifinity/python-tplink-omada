...
class APsResource:
    ...
    def all(
        ...
        **params: Any,
        ...
        return cast(
            dict[str, Any],
            self.client.devices.all(site_id=site_id, page=page, page_size=page_size, deviceType="ap", **params),
        )
    ...
    def get_by_mac(self, *, site_id: str, mac: str) -> dict[str, Any]:
        ...
        response = cast(
            dict[str, Any],
            self.client.devices.all(site_id=site_id, searchKey=normalized_mac, deviceType="ap"),
        )
    ...
    def get_by_name(self, *, site_id: str, name: str) -> dict[str, Any]:
        response = cast(
            dict[str, Any],
            self.client.devices.all(site_id=site_id, searchKey=name, deviceType="ap"),
        )
        items = self._extract_items(response)
    ...
    def get_by_serial(self, *, site_id: str, serial: str) -> dict[str, Any]:
        response = cast(
            dict[str, Any],
            self.client.devices.all(site_id=site_id, deviceType="ap"),
        )
        items = self._extract_items(response)
        for item in items:
            if not isinstance(item, dict):
            ...
            if item.get("sn") == serial:
                ...
                augment_device_status_meanings(matched)
                return matched
        raise DeviceNotFoundError(f"AP with serial '{serial}' not found in site '{site_id}'")
