...
def test_list_onus_calls_expected_path_and_params() -> None:
    ...
    payload = resource.list_onus(site_id="s1", olt_mac="aa:bb:cc:dd:ee:ff", pon_port="GPON 1/1/1")

    assert "result" in payload
    assert client.calls[0] == (
        "GET",
        "/openapi/v1/sites/s1/olts/AA-BB-CC-DD-EE-FF/pon/onu-management/informations/list",
        {"ponPort": "GPON 1/1/1"},
    )
...
def test_get_onu_detail_calls_expected_path_and_key_param() -> None:
    ...
    payload = resource.get_onu_detail(site_id="s1", olt_mac="AA-BB-CC-DD-EE-FF", onu_key="1-1-1_0")

    assert payload["result"]["onuOpticalLinkInformation"]["receivedOpticalPower"] == -7.06
    assert client.calls[0] == (
        "GET",
        "/openapi/v1/sites/s1/olts/AA-BB-CC-DD-EE-FF/pon/onu-management/informations/detail/get",
        {"key": "1-1-1_0"},
    )
...
def test_api_path_rewrite_is_used() -> None:
    ...
    resource.list_onus(site_id="s1", olt_mac="aa:bb:cc:dd:ee:ff", pon_port="GPON 1/1/1")

    assert client.calls[0] == (
        "GET",
        "/openapi/v1/omadac-1/sites/s1/olts/AA-BB-CC-DD-EE-FF/pon/onu-management/informations/list",
        {"ponPort": "GPON 1/1/1"},
    )
